...
- **只能操作 myas 安装的数据库**：判断依据是 `~/.myas/instances.tsv` 里有该实例的登记、
  并且 `myas list` 能看到它。由人工、其他工具或别的产品安装的数据库（例如
  `192.168.23.13` 上的 `tpcc`、`/data/yashan` 下的 `yashandb` 遗留目录集）**一律不得
  停止、启动、删除或改配置**，不管它看起来像不像残留；只能只读盘点并报告给用户。
